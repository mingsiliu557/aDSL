...
12. `INDETERMINATE / MESH_INVALID` is a narrow exception: invalid volume elements are measured, but their geometric cause is NOT established and structural performance is unverified. Use supplied element counts, local bounds and source candidates for a bounded local geometry hypothesis only. If localization is absent or no defensible edit exists, leave it unresolved. Never delete bad mesh elements, relax tolerances/thresholds, or alter checker, mesher or solver settings. Recheck every configured checker; becoming unavailable again is not improvement, and newly revealed physical failures must not be hidden.
