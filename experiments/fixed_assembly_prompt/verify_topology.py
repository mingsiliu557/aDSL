...
def measure(saved,output,profile=DEFAULT_PROFILE,*,asset_root=None,case_id='SF13',max_rounds=2):
    if not 1 <= max_rounds <= 5:
        raise ValueError('max_rounds must be between 1 and 5')
    profile=profile.resolve()
    profile_hash=file_hash(profile)
    output.mkdir(parents=True,exist_ok=False)
    generate=asset_root or saved/'generate'
    ...
    original_input=(read_json(generate/'runtime_config.json')['request'] if asset_root
                    else read_json(saved/'input.json'))
    record={'source_origin':str(generate/'source.py'),'source_sha256':file_hash(output/'source.py'),
        ...
        'checker_spec':checker_spec().model_dump(),'case_id':case_id,'initial_generations':0,'max_source_repairs':max_rounds-1,
        'max_rounds':max_rounds,'api_profile':profile.stem,'llm_config':str(profile),
...
async def repair(output,*,review_all=False):
    ...
    if not review_all and not _actionable_findings(CheckerRun(checker_spec(),baseline,output,())):
    ...
    max_rounds=record.get('max_rounds',2)
    req=record['requirement']+ ('\nCURRENT RUN OVERRIDE: this is an existing-asset repair check, not generation.'
        ...
        f' Existing geometry mode is off. At most {max_rounds-1} source edits, {max_rounds} evaluations, no Planner.'
        ' Apply minimal body/interface/assembly changes using measured evidence; preserve original task and frozen conditions.')
    evidence_files=[{'path':str((output/'boundary_localization.json').resolve()),
        'purpose':'Initial boundary positions and source ranges; not proof of Boolean cause or disconnected interfaces.',
        'source_sha256':record['source_sha256'], 'version_role':'initial_source_only'}]
    request=ObjectRequest(req,output,record.get('case_id','SF13')+'_assembly_topology',max_rounds=max_rounds,fixed_assembly=config,
