#!/usr/bin/env bash
# Consume the separately managed local proxy; never start/stop it here.
set -eo pipefail
...
print('CLIProxy local health/model check passed; credentials not logged.', flush=True)
PY
"$lms/envs/adsl/bin/python" -u experiments/fixed_assembly_existing/run.py "$@"
